...
calibration_and_setup() {
  ...
  echo "SUBSYSTEM==\"input\", ATTRS{idVendor}==\"0b9a\", ATTRS{idProduct}==\"016a\", ACTION==\"add\", RUN+=\"/bin/bash -c 'evdev-joystick --e %E{DEVNAME} -a 0 -f $x_fuzz -m $x_min -M $x_max ; evdev-joystick --e %E{DEVNAME} -a 1 -f $y_fuzz -m $y_min -M $y_max'\"" | sudo tee /etc/udev/rules.d/99-guncon2.rules
